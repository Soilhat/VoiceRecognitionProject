...
modelForest
```
Here we specify with the parameter *na.action = na.omit* to omit the unknown values. We find an error rate of 2% so a __98 %__ accuracy on the training set, the result is similar as the model with Logistic Regression.
...
```
We find a 98% accuracy on the test set.